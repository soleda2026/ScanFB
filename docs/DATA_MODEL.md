...
Lifecycle: Nguoi dung enroll group qua existing authoritative Add path; group duoc giu qua cac lan mo app va co the bat/tat ma khong doi authoritative identity. Optional future discovery/synchronization, neu duoc phe duyet rieng, co the dung cung source-neutral model.
...
- Enrollment khong chung minh current Facebook membership, access permission, Safari login validity hoac future post availability.
...
- `SQLiteBatchRepository` open/create explicit local SQLite path, enable foreign keys, initialize/validate schema metadata, close, satisfy `BatchRepository` bang transactional `SaveBatch`, va expose concrete-only `LoadBatch` cho deterministic fail-closed reconstruction; load/list tren `BatchRepository` van deferred.
- SQLite reconstruction dung `BatchRecordID` authoritative, explicit position columns, canonical RFC3339Nano UTC timestamp strings decoded ve stored timezone, strict boolean 0/1, schema validation va final `BatchRecord.Validate`; malformed data tra zero record va error.
