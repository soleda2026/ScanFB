...
- `SQLiteBatchRepository` chi open/create explicit local SQLite path, enable foreign keys, initialize/validate empty schema metadata va close. Vi chua co `SaveBatch`, type nay khong satisfy `BatchRepository`.
