...
- Phase 10B2d: future acquisition-probe tests must cover fixed-script forbidden tokens, exact current-tab targeting, direct `/usr/bin/osascript`, strict finite bounds, HTTPS URL validation, TCC and Safari developer-setting errors, process start/nonzero/timeout/cancellation, zero result on failure, no browser mutation/private-state/network behavior and one separately approved redacted manual validation. No production selector or private Facebook fixture belongs in this slice.
