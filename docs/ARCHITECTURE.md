...
- `internal/bridge`: bridge-facing Go package for bounded typed `core_readiness`, Phase 9E2 persistent watched-group operations and Phase 11C2 `prepared_group_scan`. The scan handler reloads one authoritative active group and wires the in-memory prepared collector to Phase 11A; requests expose no raw path, SQL, collection or cursor authority.
...
- `macos/ScanFBApp`: native SwiftUI macOS app shell from Phase 8B plus the Phase 8 fixture screens, readiness status, persistent Watched Groups presentation and Phase 11C2 session-only structured prepared-post form/count summary. Swift owns no storage path, SQLite access, cursor policy, scan rules or durable prepared-scan state.
...
Phase 11C1 implements that Go adapter in `internal/facebook`. `PreparedSnapshotCollector` owns a defensive copy of caller bytes, or only an oversize flag when input exceeds 1 MiB; no file/stdin/clipboard source exists. Its strict JSON v1 decoder rejects unknown fields, duplicate keys at any object depth, trailing/multiple values, malformed/types, zero/over-100 posts and finite per-field byte violations. It requires explicit RFC3339/RFC3339Nano `created_at` with exact `+07:00`, maps user values without inference, supplies group ID/name from the active enrolled `GroupCollectionRequest` and `CapturedAt` from `ScanWindow.ScanStarted()`, then calls Phase 10A `ExtractPreparedPage` exactly once. The adapter returns one existing `GroupCollectionResult` and is replaceable behind unchanged Phase 11A. It adds no application-domain dependency reversal, UI/bridge/file/clipboard, browser/network, persistence, cursor, retry, scheduler or concurrency edge.

Phase 11C2 adds one bounded `prepared_group_scan` bridge operation and one SwiftUI form on the Groups screen. Swift encodes 1-100 ordered typed post rows into nested prepared-snapshot JSON v1, with exact `Asia/Ho_Chi_Minh` `+07:00` timestamps; group/capture/profile/mode fields are not part of that nested payload. The helper reloads the group from Go-owned WatchedGroup persistence, rejects missing/inactive state, constructs one current-day window from the explicit caller action timestamp, uses built-in MacBook plus default `hcm`, then calls Phase 11C1 and Phase 11A once. The response is bounded count-only data. No input/result/attempt is persisted, no cursor method is called, and no Facebook/Safari/network/file/clipboard/retry/scheduler/batch-five edge is added.
