...
SwiftUI is the approved native macOS presentation technology for Phase 8. Phase 8C adds only static fixture-driven Overview presentation at `macos/ScanFBApp`; Phase 8D adds only fixture-driven buyer Leads presentation; Phase 8E adds only fixture-driven Dry Run review presentation. The Go core remains authoritative for rules, deduplication, blocklist behavior, batch evaluation, lead aggregation, persistence snapshots, reason codes and deterministic decisions.
