...
Trang thai hien tai: Go core da co deterministic domain/rules/dedup/blocklist/application/orchestration foundations, SQLite save/load foundations cho completed batch snapshots, Phase 9A in-memory lifecycle state machine cho mot production-shaped batch dung 5 group, Phase 9B in-memory WatchedGroup management voi so group khong gioi han, Phase 9C in-memory active-only round-robin selector cho dung 5 group, Phase 9D mapping tu mot selection da duoc phe duyet sang lifecycle inputs do caller cung cap, Phase 9E1 session-only Watched Groups UI va typed bridge dung truc tiep Phase 9B/9C da hoan tat automated verification va user-guided manual verification, va Phase 9E2a da approve production Application Support location cung dedicated WatchedGroup-state SQLite schema v1 cho future implementation. Phase 10A co fixture-only prepared-page extraction boundary tao `RawPost` deterministic, va Phase 10B1 co Safari-only user-triggered active-tab acquisition boundary da duoc manual validate tren mot user-prepared group page. Phase 10B2a one-page reconnaissance ket luan Safari `tab.source()` khong chua du post-level DOM evidence de implement selector fail closed. Phase 10B2d rendered-DOM acquisition va Phase 10B2f typed redacted preservation path deu hoat dong. Phase 10B2g live closeout ghi nhan 2 semantic article candidates va deterministic traversal, nhung 0 approved permalink, body, author, machine timestamp va complete-post evidence. Phase 10B2b production selector van BLOCKED; current Safari selector investigation da dong va khong duoc ha selector standard de tiep tuc. Phase 8B da tao native SwiftUI macOS app shell buildable tai `macos/ScanFBApp/`. Phase 8C them static sample Overview dashboard cho mot completed batch minh hoa bang du lieu synthetic. Phase 8D thay placeholder `Leads` bang fixture-only buyer lead tabs/cards voi du lieu synthetic va action disabled. Phase 8E thay placeholder `Dry Run` bang fixture-only review tabs/cards cho bai duoc chon, can xem xet va da loai. Phase 8F thay placeholder `Blocklist` va `Cài đặt` bang fixture-only presentation screens voi du lieu synthetic, read-only va action disabled. Phase 8G them in-memory interaction state cho fixture Leads, va Phase 8H gioi han states con `Mới`, `Đã xem`, `Bỏ qua` voi action `Tương tác` handoff URL nguon mau sang trinh duyet macOS mac dinh. Phase 8I.2 them bridge slice dau tien chi cho Go core readiness bang local subprocess helper, voi response gioi han `schema_version`, `readiness_status` va `core_identity`; Phase 8I.2a package helper vao Debug app bundle tai `Contents/Helpers/scanfb-bridge-helper`. Chua co production scan workflow/execution, production Facebook DOM selectors, browser extension, watched-group/cursor persistence runtime, lead/search bridge hoac production database wiring.
...
SwiftUI chi la presentation layer; khong reimplement business rules, reason codes, summaries, deduplication, blocklist outcomes hoac SQLite access. Phase 8I.2 implements `core_readiness`, va Phase 9E1 them bon operation hep `watched_groups_list`, `watched_groups_add`, `watched_groups_set_active` va `watched_groups_next_five` qua cung local subprocess helper. Swift `WatchedGroupsStore` chi so huu snapshot va cursor cua current app session; moi bridge call gui snapshot do sang Go, noi `WatchedGroupCollection` duoc tai tao va authoritative Phase 9B/9C validation, mutation va selection duoc ap dung. Debug builds package the helper at `Contents/Helpers/scanfb-bridge-helper`; app runtime resolves only that bundle-relative executable and fails closed when it is absent. No PATH search, shell fallback, hidden network, Facebook, persistence write, lead/search bridge or broad command bus exists.

Phase 9E2a chi approve future persistence architecture, chua implement storage. Go helper se tu resolve `<user-application-support>/com.soleda.ScanFB/watched-groups.sqlite3`; completed batches dung database rieng `completed-batches.sqlite3`. Swift khong thay path va khong so huu persistence. Existing completed-batch schema v1 khong doi.
